...
def filter_and_adjust_bounding_boxes(bounding_boxes):
    print("filter func")
    if not bounding_boxes:  # Check if bounding_boxes is empty
        print("Skipping empty page: No bounding boxes found.")
        return []
...
def process_directory(image_dir, contour_dir, output_dir, padding=20, min_width=100, threshold=0.2, margin=0.1, visualize=False):
    ...
    for image_path in image_paths:
        ...
        if os.path.exists(contour_file_path):
            ...
            if not bounding_boxes:  # Skip empty pages
                print(f"Skipping {image_path}: No bounding boxes detected.")
                continue
